...
app.get('/v1/colectivos/positions', (c) => {
  ...
  return serve(c, {
    ...
    live: async () => {
      ...
      // El filtro por línea se hace en el cliente por proximidad al recorrido:
      // el feed identifica las líneas con IDs internos no mapeables de forma
      // confiable (ver docs/api-notes.md), así que acá se devuelven todos.
      return lines.length ? all.filter((v) => lines.includes(v.lineLabel)) : all;
    },
